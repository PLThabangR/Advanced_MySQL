...
select x.order_id,x.order_date,y.first_name,y.points from orders as x join customers as y on x.customer_id = y.customer_id
where y.points >1000;

select * from products;
select * from order_items;

select * from products as p inner join order_items as oi on p.product_id = oi.product_id;

select order_id,order_date,'Active' as Status from orders
where year(order_date) >= '2019'
union
select order_id,order_date,'Achived' as Status from orders
where year(order_date) < '2019';
...
create table invoice_archived as select i.invoice_id,i.number,c.name,i.invoice_total,i.payment_total from invoices as i join clients as c on i.client_id= c.client_id
where payment_total > 0
order by c.name asc;

select * from invoice_archived;
select max(invoice_total) as max,min(invoice_total) as min ,count(invoice_total) as num_of_invoices,sum(invoice_total) as sum   from invoices;
...
update customers 
 ...
 update orders 
 set comment ="Gold customer" where customer_id in (select customer_id from customers 
where points > 3000);

select date , sum(amount) as total  from payments
group by date
order by date;
...
use sql_store;
select * from products
where unit_price > any( select unit_price from products where unit_price > 3);
